...
import { EstimateFeeInput, EstimateFeeResponse, Transaction } from "../shared/types";
...
const estimateFeeHelper = async (input: EstimateFeeInput) => {
  const response = await axios.post<EstimateFeeResponse>("/transactions/transfer/estimateFee", input);
...
const validateAddressMutationHelper = async ({ address, blockchain }: { address: string; blockchain: string }) => {
  const { data } = await axios.post<{}, { data: { isValid: boolean } }>("/transactions/validateAddress", {
    address,
    blockchain,
  });
...
export const useValidateAddressMutation = () => useMutation(validateAddressMutationHelper);
...
const createTransferHelper = async (bodyParams: {
  ...
  const response = await axios.post<{ challengeId: string }>("/transactions/transfer", bodyParams);

  return response.data;
};

export const useCreateTransferMutation = () => useMutation(createTransferHelper);
