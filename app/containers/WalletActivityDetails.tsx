...
import { findChipColor, formatDate, getTransactionOperation, roundNum } from "@/app/shared/utils";
...
import { LoadingWrapper, Content, BackButton, CopyButton } from "@/app/components";
...
export const WalletActivityDetails: React.FC<WalletActivityDetailsProps> = ({ walletId, transactionId }) => {
  const router = useRouter();
  const { data: transaction } = useTransactionQuery(transactionId);
  const { data: wallet } = useWallet(walletId);
  const { operation } = getTransactionOperation(wallet?.data.wallet.address ?? "", transaction);
        ...
        <Content>
          ...
          <div className="space-y-2 grow">
            ...
            <TextField
              ...
              readOnly
            />
            <TextField value={blockchainNames[transaction?.blockchain ?? ""]} label="Network" readOnly />
            <TextField
              readOnly
              startDecorator={
            ...
            <TextField
              ...
              startDecorator={
                transaction?.state && (
                  <Chip color={findChipColor(transaction.state)} size="md" variant="solid">
                    {transaction.state}
                  </Chip>
                )
              }
            />
            <TextField value={`${date.toLocaleTimeString()} ${formatDate(date)}`} label="Date" readOnly />
          </div>
