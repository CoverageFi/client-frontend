...
import { useWallet, useWalletBalances, useFaucetDripMutation } from "@/app/axios";
...
export const WalletDetails: React.FC<WalletDetailsProps> = ({ id }) => {
  ...
  }, [dripFaucet, walletData?.data.wallet.address, walletData?.data.wallet.blockchain]);
  ...
  const isWalletEmpty = !isLoading && balanceData?.data.tokenBalances.length === 0;
  ...
  return (
    <>
      <LoadingWrapper isLoading={isLoading}>
        ...
        <Typography className="text-3xl text-center max-w-80 mx-auto text-light" fontWeight={700} level="title-lg">
          {mainBalance?.amount ?? "0"} {mainBalance ? nativeTokenInfo.name : blockchainInfo.nativeTokenName}
        </Typography>
        <div className="flex flex-row justify-center gap-4 py-2">
          <Button startDecorator={<ArrowDownIcon width={16} />} onClick={() => router.push(`/wallets/${id}/deposit`)}>
            Deposit
          </Button>
          {isWalletEmpty ? (
            <Button
              disabled={dripFaucet.isLoading || dripFaucet.isSuccess || dripFaucet.isError}
              variant="outlined"
            ...
            <Button startDecorator={<ArrowUpIcon width={16} />} onClick={() => router.push(`/wallets/${id}/send`)}>
              Send
            </Button>
          )}
        </div>
        {isWalletEmpty && (
          <>
            <Image alt="no tokens" src={`/NoTokens.svg`} height={120} width={120} className="mx-auto" />
            <span className="text-center font-semibold text-gray-400">
              {(dripFaucet?.status === "idle" || dripFaucet?.status === "loading") && (
              ...
              {dripFaucet.isSuccess && (
                <Typography level="title-lg" className="text-inherit">
                  Your funds have been requested, please wait up to 10 seconds for the transaction to settle.
                </Typography>
              )}
              ...
              {dripFaucet.isError && (
                <Typography level="title-lg" className="text-inherit">
                  ...
                  <a href="https://faucet.circle.com" className="no-underline text-blue-500" target="_blank">
                    public faucet.
                  </a>
                </Typography>
              )}
            ...
            <TabPanel value={0} className="px-0">
              <div className="flex grow w-full flex-col gap-2">
                {balanceData?.data.tokenBalances.map((token: TokenBalance) => (
                  <TokenCard key={token?.token.name} amount={token?.amount} token={token.token} />
                ))}
              </div>
            </TabPanel>
